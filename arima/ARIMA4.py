...
def visualize_power_usage_by_zone(consumption):
    ...
    # Monthly Power Usage
    consumption['Months'] = consumption.index.month_name()
    consumption['Season'] = consumption.index.to_period('M').strftime('%B')
    consumption['Weekdays'] = consumption.index.strftime('%A')
    ...
    for i, zone in enumerate(['Zone1', 'Zone2', 'Zone3']):
        consumption.boxplot(column=f'PowerConsumption_{zone}', by='Months', ax=axes[i], grid=False, vert=False, showfliers=False)
    ...
    fig, axes = plt.subplots(nrows=3, ncols=1, figsize=(10, 15))
    for i, zone in enumerate(['Zone1', 'Zone2', 'Zone3']):
        consumption.boxplot(column=f'PowerConsumption_{zone}', by='Weekdays', ax=axes[i], grid=False, vert=False, showfliers=False)
    ...
    fig, axes = plt.subplots(nrows=3, ncols=1, figsize=(10, 15))
    for i, zone in enumerate(['Zone1', 'Zone2', 'Zone3']):
        consumption.boxplot(column=f'PowerConsumption_{zone}', by='Season', ax=axes[i], grid=False, vert=False, showfliers=False)
    ...
    for i, zone in enumerate(['Zone1', 'Zone2', 'Zone3']):
        consumption.boxplot(
        column=f'PowerConsumption_{zone}',
        by='Day_number',
        ax=axes[i],
    )
def arima_model(plot_df, zone):
    # Fit ARIMA model
    model = SARIMAX(plot_df[f'PowerConsumption_{zone}'], order=(1, 0, 0))
    ...
    # Forecast
    future = pd.DataFrame(index=pd.date_range(plot_df[f'PowerConsumption_{zone}'].index[-1] + pd.Timedelta(minutes=10), periods=30, freq='10T'))
    ...
    # Calculate RMSE
    actual_values = plot_df[f'PowerConsumption_{zone}'].iloc[-30:]
...
if __name__ == "__main__":
    # Data Preparation
    consumption = load_and_clean_data(r"C:\Users\admin\OneDrive\Desktop\Projekt\powerconsumption.csv")
