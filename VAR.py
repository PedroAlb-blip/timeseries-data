...
for col in columns_to_plot:
    ...
    # Apply differencing
    data[col + ' Diff'] = data[col + ' Log'].diff(seasonality)

    # Apply rolling mean
    data[col + ' Rolling Avg'] = data[col + ' Diff'].rolling(window=143).mean()

    # Drop NaN values
    data_to_plot = data[col + ' Rolling Avg'].dropna()
...
for col in columns_to_plot:
    if not check_stationarity(data[col + ' Rolling Avg']):
